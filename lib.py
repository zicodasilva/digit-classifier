...
def load_mnist_dataset(input_img_size: int = 28, batch_size: int = 64, use_multi_label = True) -> Tuple[DataLoader, DataLoader]:
    # Get data.
    ...
    for digit in range(10):
        ...
        if use_multi_label:
            y_outs = [0]*10
            y_outs[digit] = 1
            y.append([y_outs] * img_train_data.size(0))
            y_val.append([y_outs] * img_test_data.size(0))
        else:
            y.append([digit] * img_train_data.size(0))
            y_val.append([digit] * img_test_data.size(0))
    # TODO: Need to add blank image dataset here so that we can classify images without digits.
    ...
    y = torch.tensor(functools.reduce(lambda a, b: a+b, y))
    ...
    y_val = torch.tensor(functools.reduce(lambda a, b: a+b, y_val))
    if not use_multi_label:
...
def calc_grad(xb: torch.Tensor, yb: torch.Tensor, model: torch.nn.Module):
    preds = model(xb)
    if yb.shape[1] > 1:
        loss = binary_cross_entropy_loss(preds, yb)
    else:
...
def calc_batch_accuracy(model: torch.nn.Module, X: torch.Tensor, Y: torch.Tensor) -> Tuple[float, float]:
    _, max_idx_class = model(X).softmax(dim=1).max(dim=1)  # [B, n_classes] -> [B], # get values & indices with the max vals in the dim with scores for each class/label
    acc = (max_idx_class == Y.squeeze()).sum().item() / X.size(0)
    batch_loss = 0.0
    with torch.no_grad():
        batch_loss = cross_entropy_loss(model(X), Y).item()
    return batch_loss, acc

def calc_multi_batch_accuracy(model: torch.nn.Module, X: torch.Tensor, Y: torch.Tensor, threshold = 0.5):
    pred = model(X)
    acc = ((pred.sigmoid() > threshold) == Y.bool()).float().mean()
    batch_loss = 0.0
    with torch.no_grad():
        batch_loss = binary_cross_entropy_loss(pred, Y).item()
    return batch_loss, acc
...
def validate_epoch(model: torch.nn.Module, dataset: DataLoader) -> Tuple[float, float]:
    ...
    for xb, yb in dataset:
        if yb.shape[1] > 1:
            l, a = calc_multi_batch_accuracy(model, xb, yb)
        else:
...
def classify(model: torch.nn.Module, img_path: Union[str, Path], n_dim: int, use_multi_label = True, threshold = 0.5) -> torch.Tensor:
    img_data = transforms.ToTensor()(Image.open(img_path))
    pred = model(img_data.view(-1, n_dim))
    if use_multi_label:
        indices = (pred.sigmoid() > threshold)
        possible_values = torch.arange(10)
        digit = possible_values[indices.squeeze()]
    else:
        digit = torch.argmax(torch.nn.Softmax(dim=1)(pred), dim=1)
